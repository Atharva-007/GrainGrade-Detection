...
AI Grain Grade is a Streamlit application for finger millet (ragi) quality grading. It combines image-derived physics proxies, a rule engine, local RAG over grading documents, and an OpenAI-compatible Qwen vision endpoint.
...
- Calls a configured Qwen vision provider when cloud inference is enabled.
- Applies deterministic safety and grading rules before showing the final result.
- Stores human corrections as feedback examples for later active learning.
...
- `src/ai_grain_grade/vision_rag_pipeline.py` - vision model calls, safety gate, RAG-guided grading, fallback logic.
...
- `src/ai_grain_grade/rag_engine.py` - local Markdown chunking and retrieval.
- `src/ai_grain_grade/lora_finetune.py` - feedback storage and LoRA training scaffolding.
...
|-- data/feedback/                 # Example and local feedback records
...
|-- models/                        # Local model/adaptor outputs
...
- `QWEN_VL_PROVIDER` - `dashscope`, `siliconflow`, `ollama`, or `custom`.
...
Some tests use local indexes and deterministic fallbacks. Cloud provider tests mock HTTP calls and do not require a real API key.

...
def configuration_help_text(api_key_source: str, vertex_mode: bool) -> str:
    if vertex_mode:
        return (
            "Vertex AI mode uses Google Cloud project/location configuration and "
            "Application Default Credentials. If cloud calls fail, confirm "
            "`GOOGLE_CLOUD_PROJECT`, `GOOGLE_CLOUD_LOCATION`, and auth setup."
        )
    if api_key_source == "VERTEX_API_KEY":
        return (
            "`VERTEX_API_KEY` is being used as a legacy fallback name for a Gemini API key. "
            "If this is a Gemini Developer API key, prefer renaming it to `GEMINI_API_KEY`. "
            "If you intended true Vertex AI mode, set `GOOGLE_GENAI_USE_VERTEXAI=true` "
            "and configure Google Cloud auth instead of an API key."
        )
    if api_key_source == "GEMINI_API_KEY":
        return (
            "This app expects a Gemini Developer API key in `GEMINI_API_KEY` for "
            "non-Vertex mode. If calls fail with `SERVICE_DISABLED`, enable the "
            "Generative Language API for the key's Google Cloud project or create "
            "a new Gemini key from Google AI Studio."
        )
    return (
        "No cloud credential detected. The app can still run in local fallback mode, "
        "but the heuristic grader is lower-confidence than Gemini Vision."
    )
...
def format_external_error(exc: Exception) -> str:
    ...
    if "SERVICE_DISABLED" in text or "Gemini API has not been used" in text:
        return (
            "Gemini API is disabled for the Google project attached to this API key. "
            "Enable the Generative Language API for that project or replace the key "
            "with a working `GEMINI_API_KEY`."
        )
    if "PERMISSION_DENIED" in text:
        return (
            "Google returned PERMISSION_DENIED for the configured cloud credential. "
            "Check whether the API key/project is authorized for Gemini or Vertex AI."
        )
    if "API key not valid" in text or "API_KEY_INVALID" in text:
        return "The configured Gemini API key is invalid. Check `.env` and rotate the key if needed."
...
def retrieve_rag_context(query: str, k: int) -> tuple[str, list[dict]]:
    ...
    client = None
    if configured_for_genai(api_key, vertex_mode):
        try:
            client = create_genai_client(api_key, vertex_mode)
        except Exception:
            client = None

    hits, retrieval_mode = rag.retrieve_with_mode(client, query, k=k)
    formatted = rag.format_retrieved(hits)
    meta = [{"source": h[0].source, "title": h[0].title, "score": h[1],
             "preview": h[0].text[:200], "retrieval_mode": retrieval_mode} for h in hits]
...
def local_grade_image(image_bytes: bytes) -> dict:
    ...
    if grade == "A":
        margin = min(
            max(0.0, (5.0 - off_tone_percentage) / 5.0),
            max(0.0, (0.05 - defect_fraction) / 0.05),
            max(0.0, (uniformity_score - 0.72) / 0.28),
        )
        confidence = round(0.32 + 0.33 * margin, 2)
    elif grade == "B":
        margin = min(
            max(0.0, (18.0 - off_tone_percentage) / 18.0),
            max(0.0, (0.16 - defect_fraction) / 0.16),
            max(0.0, (uniformity_score - 0.45) / 0.55),
        )
        confidence = round(0.30 + 0.28 * margin, 2)
    else:
        evidence = max(
            min(1.0, off_tone_percentage / 18.0),
            min(1.0, defect_fraction / 0.25),
            min(1.0, max(0.0, 0.45 - uniformity_score) / 0.45),
        )
        confidence = round(0.28 + 0.24 * evidence, 2)
    ...
    return {
        ...
        "confidence": confidence,
        "remarks": (
            "Local fallback result because Gemini Vision was unavailable. "
            "Confidence is heuristic-only and capped below cloud-model confidence."
        ),
...
api_key = get_api_key()
api_key_source = get_api_key_source()
vertex_mode = use_vertex_ai()
...
with st.sidebar:
    ...
    if vertex_mode:
        ...
        st.caption(configuration_help_text(api_key_source, vertex_mode))
    elif api_key_source == "VERTEX_API_KEY":
        st.warning(f"Legacy key loaded from `VERTEX_API_KEY` (...{api_key[-6:]})")
        st.caption(configuration_help_text(api_key_source, vertex_mode))
    elif api_key:
        st.success(f"GEMINI_API_KEY loaded (...{api_key[-6:]})")
        st.caption(configuration_help_text(api_key_source, vertex_mode))
    else:
        st.warning("No cloud credential loaded")
        st.caption(configuration_help_text(api_key_source, vertex_mode))
...
with tab_grade:
    ...
    with col_out:
        ...
        if run and uploaded is not None:
            ...
            if use_rag and genai_configured:
                with st.spinner("Retrieving from knowledge base…"):
                    try:
                        rag_context, rag_meta = retrieve_rag_context(RAG_QUERY, rag_k)
                    except Exception as e:
                        rag_warning_message = f"RAG retrieval failed; continuing without RAG: {format_external_error(e)}"
            if rag_warning_message:
                st.warning(rag_warning_message)
            ...
            if genai_configured:
                try:
                    ...
                    if should_use_local_fallback(e):
                        fallback_reason = format_external_error(e)
                    else:
                        st.error(f"Grading failed: {format_external_error(e)}")
                        st.stop()
            else:
                fallback_reason = (
                    "No usable Gemini or Vertex AI configuration was detected. "
                    "Running the local heuristic fallback instead."
                )

            if result is None:
                fallback_used = True
                st.warning(f"{fallback_reason}")
            ...
            if fallback_used:
                st.caption(
                    "Analysis mode: local heuristic fallback. Confidence is not a Gemini model score "
                    "and is intentionally capped."
                )
            ...
            if rag_meta:
                rag_mode = rag_meta[0].get("retrieval_mode", "unknown")
                if rag_mode == "local-keyword":
                    st.caption("RAG mode: local keyword retrieval over indexed docs.")
                with st.expander(f"RAG context used ({len(rag_meta)} passages · {rag_mode})"):
                    for m in rag_meta:
                        st.markdown(
            ...
            st.info(
                "Cloud model is not configured. Upload an image to use the local fallback, "
                "or add `GEMINI_API_KEY` / Vertex AI config for Gemini Vision."
            )
        else:
